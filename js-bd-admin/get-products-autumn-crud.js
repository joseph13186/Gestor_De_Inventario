...
//Verano
function cargarProductosAutumn() {
    $.ajax({
        url: 'http://127.0.0.1:8000/php-bd/products-query-autumn.php',  // Ruta a tu script PHP
        type: 'GET',  // Método GET
        dataType: 'json',  // Esperamos que nos devuelvan datos en formato JSON
        success: function(data) {
            // Limpiamos la tabla antes de cargar nuevos datos
            var table = $('#otonio-dt').DataTable();
            table.clear();
            ...
            table.draw(false); // Solo llamamos draw una vez
             ...
             $('#otonio-dt tbody').off('click', '.btn-editar').on('click', '.btn-editar', function () {
                const id = $(this).data('id');
                // 1. Buscar datos del producto con ese ID
                obtenerInfoProducto(id);
            });

            $('#otonio-dt tbody').off('click', '.btn-eliminar').on('click', '.btn-eliminar', function () {
                //const id = $(this).data('id');
...
$(document).ready(function() {
    $('#otonio-dt').DataTable();  // Solo una vez
    cargarProductosAutumn();               // Carga dinámica desde PHP
});
